  ...
  return <Portal>
    <dialog ref={dialog} class="workspace-settings" aria-label={t()("settings.title")}
      ...
      <aside class="preferences-nav">
        ...
        <footer class="sidebar__account"><button type="button" class="sidebar__identity" title={`${session.user().username} · ${session.user().admin ? text("管理员", "Administrator") : text("成员", "Member")}`} aria-label={`${session.user().username} · ${session.user().admin ? text("管理员", "Administrator") : text("成员", "Member")}`} onClick={() => selectSection("account")}><span class="member-avatar" aria-hidden="true">{session.user().username.slice(0, 2).toUpperCase()}<Show when={session.user().admin}><span class="member-avatar__dot" /></Show></span><span class="account-identity-text"><strong>{session.user().username}</strong><small class="sidebar__role">{session.user().admin ? text("管理员", "Administrator") : text("成员", "Member")}</small></span></button><button type="button" class="sidebar__settings" onClick={() => selectSection("general")} aria-label={text("设置", "Settings")} title={text("设置", "Settings")}><span innerHTML={IconGear} aria-hidden="true" /></button></footer>
      </aside>
