...
test("a stale REST save cannot overwrite another editor's content", async (t) => {
  ...
  const child = spawn(process.execPath, [path.join(__dirname, "../server.js")], {
    env: { ...process.env, PORT: String(port), TASKS_DIR: `${dir}/tasks`, CONFIG_DIR: `${dir}/config`, LOCAL_IMAGES_CLEANUP_INTERVAL: "0", TEAM_KEY: "test-team-key", COOKIE_SECURE: "false" },
    stdio: "ignore",
  });
  t.after(async () => {
    child.kill();
  ...
  // The old server has no authentication. The upgraded server requires it.
  let cookie = "";
  const login = await fetch(`${base}/auth/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: "alice", password: "test_alice", teamKey: "test-team-key" }) });
  if (login.ok) {
